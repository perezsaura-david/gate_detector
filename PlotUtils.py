...
def addSides2Image(image, sides):
    ...
    for i in range(4):
        for connection in sides[i]:
            ...
            cv2.line(image, line[0], line[1], color=color[i], thickness=2)
...
def showLabels(image, labels, camera_matrix):
    ...
    gauss_corners_img = cv2.cvtColor(gauss_corners, cv2.COLOR_GRAY2BGR)
    # PAF labels
